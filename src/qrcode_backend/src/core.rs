...
fn add_logo(qr: &mut ImageBuffer<Rgba<u8>, Vec<u8>>, logo: &[u8]) {
    ...
    while logo_size + 2 * element_size <= image_size/16 {
...
fn get_qr_element_size(qr: &ImageBuffer<Rgba<u8>, Vec<u8>>) -> usize {
    const BLACK_PIXELS: [u8; 4] = [0, 0, 0, 255];
    let size = qr.width().min(qr.height());

    let mut start = size;

    for x in 0..size{
        if qr.get_pixel(x, x).0 == BLACK_PIXELS {
            start = x;
            break;
        }
    }
    ...
    for x in start..size{
        if qr.get_pixel(start+x, start+x).0 != BLACK_PIXELS {
            element_size = x;
            break;
        }
    }
